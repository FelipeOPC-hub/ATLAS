    ...
    <section className="relative overflow-hidden bg-gradient-to-b from-brand-50/60 via-white to-white pt-28 pb-20 sm:pt-36 dark:from-slate-900/60 dark:via-slate-950 dark:to-slate-950">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 sm:px-8 lg:grid-cols-2">
        ...
        <div className="animate-fade-up">
          ...
          <p className="mt-4 text-xl font-semibold text-brand-700 sm:text-2xl dark:text-brand-400">
        ...
        <div className="relative animate-fade-up [animation-delay:150ms]">
          <div className="relative mx-auto max-w-md lg:max-w-none">
            ...
            <div className="absolute -left-4 bottom-16 w-56 rounded-2xl border border-slate-100 bg-white p-4 shadow-xl shadow-brand-900/10 animate-float sm:-left-8 lg:-left-12 dark:border-slate-800 dark:bg-slate-900">
              <div className="flex items-center gap-2">
                <div className="grid h-8 w-8 place-items-center rounded-lg bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300">
                  <CheckCircle2 className="h-5 w-5" />
                </div>
                <div>
                  <p className="text-xs font-semibold text-slate-900 dark:text-white">Servicio activo</p>
                  <p className="text-[11px] text-slate-400 dark:text-slate-500">Sucursal CABA</p>
                </div>
              </div>
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                <div className="h-full w-4/5 rounded-full bg-gradient-to-r from-accent-400 to-accent-500" />
              </div>
              <p className="mt-2 text-[11px] font-medium text-slate-500 dark:text-slate-400">80% completado</p>
            </div>

            {/* Card flotante: auditoría */}
            <div className="absolute -right-4 top-12 w-48 rounded-2xl border border-slate-100 bg-white p-4 shadow-xl shadow-brand-900/10 animate-float [animation-delay:1.5s] sm:-right-6 lg:-right-10 dark:border-slate-800 dark:bg-slate-900">
              <div className="flex items-center gap-2">
                <div className="grid h-8 w-8 place-items-center rounded-lg bg-brand-100 text-brand-600 dark:bg-brand-900/40 dark:text-brand-300">
                  <BarChart3 className="h-5 w-5" />
                </div>
                <p className="text-xs font-semibold text-slate-900 dark:text-white">Auditoría</p>
              </div>
              <div className="mt-3 flex items-end gap-1.5">
                {[40, 65, 50, 80, 72, 95].map((h, i) => (
                  <div
                    key={i}
                    className="flex-1 rounded-t bg-brand-200 dark:bg-brand-800"
                    style={{ height: `${h / 3}px` }}
                  />
                ))}
              </div>
              <p className="mt-2 text-[11px] font-medium text-slate-500 dark:text-slate-400">Reporte semanal</p>
            </div>
          </div>
        </div>
      </div>
...
function WhatIsAtlas() {
  const pillars = [
    ...
    },
    {
      ...
      color: 'bg-accent-100 text-accent-700 dark:bg-accent-900/40 dark:text-accent-300',
    },
    {
      icon: ClipboardCheck,
      title: 'Auditoría',
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {pillars.map((p, i) => (
            <div
              key={p.title}
              className="group card p-6 transition-all hover:-translate-y-1 hover:border-brand-200 hover:shadow-lg hover:shadow-brand-900/5 dark:hover:border-brand-800"
  ...
  return (
    <div
      className={`relative flex flex-col rounded-3xl border p-7 transition-all hover:-translate-y-1 hover:shadow-2xl ${
        highlighted
          ? 'border-brand-300 bg-gradient-to-b from-brand-50/80 to-white shadow-xl shadow-brand-900/10 ring-2 ring-brand-500/40 lg:-mt-4 lg:mb-4 dark:border-brand-700 dark:from-brand-950/40 dark:to-slate-900'
          : 'border-slate-200 bg-white shadow-sm hover:shadow-lg hover:shadow-brand-900/5 dark:border-slate-800 dark:bg-slate-900'
      }`}
    >
      {plan.badge && (
      ...
      <div className="mt-5 flex items-baseline gap-1">
        <span className={`text-4xl font-extrabold tracking-tight ${highlighted ? 'text-brand-700 dark:text-brand-400' : 'text-slate-900 dark:text-white'}`}>
          {plan.price}
        </span>
        {plan.price !== 'Consultar' && (
          <span className="text-sm font-medium text-slate-400 dark:text-slate-500">/mes</span>
        )}
      </div>
...
function Pricing({ onChoose }: { onChoose: (id: PlanId) => void }) {
  return (
    <section id="precios" className="scroll-mt-24 bg-slate-50 py-20 sm:py-28 dark:bg-slate-900">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="mx-auto max-w-2xl text-center">
          <span className="section-eyebrow">
            <Sparkles className="h-4 w-4" />
      ...
      <div className="mx-auto max-w-3xl px-5 sm:px-8">
        ...
        <div className="mt-12 space-y-4">
          {faqs.map((item) => (
            ...
            >
              <summary className="flex cursor-pointer items-center justify-between gap-4 px-6 py-5 text-base font-semibold text-slate-900 transition-colors hover:bg-slate-50/60 dark:text-white dark:hover:bg-slate-800/60">
                {item.q}
                <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-brand-50 text-brand-600 transition-transform group-open:rotate-45 dark:bg-brand-900/40 dark:text-brand-300">
                  <span className="text-lg leading-none">+</span>
                </span>
              </summary>
              <div className="px-6 pb-5 -mt-1 text-sm leading-relaxed text-slate-600 dark:text-slate-400">
                {item.a}
              </div>
            </details>
          ))}
